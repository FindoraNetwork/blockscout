defmodule BlockScoutWeb.TransactionView do
  ...
  """
  def gas_price(%Transaction{gas_price: gas_price}, unit) when unit in ~w(wei gwei ether fra prv)a do
  ...
  * `:include_label` - Boolean. Defaults to true. Flag for displaying unit with value.
  ...
  def value(%mod{value: value}, opts \\ []) when is_transaction_type(mod) do
    ...
    format_wei_value(value, :fra, include_unit_label: include_label?)
  end

  def format_wei_value(value) do
    format_wei_value(value, :fra, include_unit_label: false)
  end
